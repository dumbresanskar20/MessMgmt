...
import { X, Receipt, Clock, CheckCircle2, AlertCircle, Store, Maximize2 } from 'lucide-react';
...
export default function OrderHistoryModal({ isOpen, onClose }) {
  ...
  const getStatusBadge = (ord) => {
    ...
    switch (ord.order_status) {
      ...
      case 'delivered':
        return <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Delivered</span>;
      default:
          ...
          <div className="flex items-center justify-between pb-4 border-b border-stone-100">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-stone-500 font-medium">Tap any order to view large scannable QR code</p>
              </div>
            </div>
              ...
              orders.map((ord) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 bg-stone-50/90 hover:bg-amber-50/40 hover:border-amber-300 rounded-2xl border border-stone-200/80 space-y-3 transition-all cursor-pointer shadow-sm group"
                  >
                    ...
                    {isAwaiting && (
                      <div className="p-2.5 bg-amber-50 rounded-xl border border-amber-200/80 text-[11px] text-amber-900 font-semibold flex items-center gap-2">
                        ...
                        <span>Please pay ₹{ord.total_amount} at counter to receive your token.</span>
                      </div>
                    )}

                    {/* QR Code Card Thumbnail */}
                    {!isAwaiting && ord.token_number && (
                      <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-stone-200/80 group-hover:border-amber-200 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className={`relative ${isDelivered ? 'grayscale opacity-35' : ''}`}>
                            <QRCodeSVG value={qrData} size={64} level="M" />
                          </div>
                          <div className="text-[11px] text-stone-600 font-medium">
                            <span className="font-bold text-stone-900 block flex items-center gap-1">
                              {isDelivered ? '✅ Already Delivered' : 'Pickup QR Code'}
                            </span>
                            {isDelivered
                              ? 'This token has already been redeemed'
                              : 'Tap to view full-screen scannable QR code'}
                          </div>
                        </div>
